<template>
  <div class="dashboard">
    ...
    <van-grid column-num="2" :border="false" class="stats-grid">
      <van-grid-item icon="orders-o" text="待审批计划" :badge="stats.pendingPlans || 0" />
      <van-grid-item icon="logistics" text="待审批入库" :badge="stats.pendingInbound || 0" />
      <van-grid-item icon="send-gift-o" text="待审批出库" :badge="stats.pendingRequisition || 0" />
      <van-grid-item icon="bag-o" text="待发放" :badge="stats.pendingIssue || 0" />
    </van-grid>
...
async function loadStats() {
  try {
    const [planRes, inboundRes, reqRes, issueRes] = await Promise.all([
      getPlans({ status: 'pending_approval', page: 1, page_size: 1 }),
      getInboundOrders({ status: 'pending_approval', page: 1, page_size: 1 }),
      getRequisitions({ status: 'pending_approval', page: 1, page_size: 1 }),
      getRequisitions({ status: 'approved', page: 1, page_size: 1 })
    ])
