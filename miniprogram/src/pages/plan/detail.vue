  ...
  <view class="plan-detail-page">
    ...
    <view class="detail-content" v-else-if="detail">
      ...
      <DetailInfoGroup
        title="基本信息"
        :items="[
          { label: '计划编号', value: detail.plan_number },
          { label: '计划名称', value: detail.name },
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      try {
        const res = await getPlanDetail(this.id)
    ...
    async handleReject() {
      try {
        ...
        if (res.confirm) {
          await rejectPlan(this.id, { remark: res.content || '' })
