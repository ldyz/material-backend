<template>
  <view class="requisition-detail-page">
    ...
    <view class="detail-content" v-else-if="detail">
      <DetailInfoGroup
        title="基本信息"
        :items="[
          { label: '领料单号', value: detail.requisition_number },
          { label: '领料类型', value: detail.type },
...
export default {
  ...
  methods: {
    async fetchDetail() {
      this.loading = true
      try {
        const res = await getRequisitionDetail(this.id)
    ...
    async handleApprove() {
      try {
        await approveRequisition(this.id, {})
    ...
    async handleReject() {
      try {
        ...
        if (res.confirm) {
          await rejectRequisition(this.id, { remark: res.content || '' })
